---
name: debt-analyzer
description: Use this agent when you need to perform Phase 1 analysis in the debt payoff workflow. This agent should ONLY be invoked by the debt-payoff-orchestrator agent, not standalone.\n\nExamples:\n\n<example>\nContext: The debt-payoff-orchestrator is beginning a debt analysis workflow after receiving user's debt inventory.\n\nuser: "I have three debts: a credit card with $5,000 at 18.99% APR with $150 minimum, an auto loan with $12,000 at 5.5% APR with $350 minimum, and a personal loan with $3,000 at 12% APR with $100 minimum. I make $5,000 per month and have $500 in emergency savings."\n\norchestrator: "I need to analyze this debt data to calculate metrics and assess risk before generating payoff strategies. Let me use the Task tool to invoke the debt-analyzer agent."\n\n<commentary>\nThe orchestrator recognizes this is Phase 1 of the workflow and uses the debt-analyzer agent to process the raw debt inventory into structured analytical data.\n</commentary>\n</example>\n\n<example>\nContext: The debt-payoff-orchestrator receives debt data in JSON format and needs baseline analysis.\n\nuser: {"debts": [{"name": "Chase Card", "balance": 8000, "apr": 22.99, "minimum": 200}], "monthlyIncome": 4500, "emergencyFund": 0}\n\norchestrator: "Before I can compare payoff strategies, I need the debt-analyzer to calculate total debt, weighted APR, debt-to-income ratio, and flag any high-risk debts. Using the Task tool now."\n\n<commentary>\nThe orchestrator always starts with debt-analyzer to get comprehensive metrics before proceeding to strategy comparison.\n</commentary>\n</example>\n\n<example>\nContext: User asks directly for debt analysis, but this should be routed through orchestrator.\n\nuser: "Can you analyze my debt and tell me the best payoff strategy?"\n\nassistant: "I'll use the Task tool to launch the debt-payoff-orchestrator agent, which will coordinate the full debt analysis workflow including invoking the debt-analyzer for calculations."\n\n<commentary>\nEven though the user mentioned analysis, the debt-analyzer should not be called directly. The orchestrator manages the workflow and will invoke debt-analyzer as Phase 1.\n</commentary>\n</example>
model: sonnet
---
